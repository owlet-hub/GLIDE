...
__global__ void
sample_kernel(const float *data, float *sample_data, uint32_t *sample_ids, uint32_t dim, uint32_t sample_num) {
    ...
    for (uint32_t i = threadIdx.x; i < dim; i += blockDim.x) {
        sample_data[point_id * dim + i] = data[sample_id * dim + i];
    }
}
...
sample_for_large_kernel(const uint8_t *data, float *sample_data, uint32_t *sample_ids, uint32_t dim,
    ...
    for (uint32_t i = threadIdx.x; i < dim; i += blockDim.x) {
        sample_data[point_id * dim + i] = static_cast<float>(data[sample_id * dim + i]);
    }
}

template<uint32_t MAX_CENTROID>
__global__ void
define_partition_kernel(const float *data, float *centroids, uint32_t num, uint32_t dim, uint32_t centroid_num,
...
define_partition_for_large_kernel(const uint8_t *data, float *centroids, uint32_t batch_size, uint32_t dim,
    ...
    for (uint32_t i = threadIdx.x; i < dim; i += blockDim.x) {
        point_buffer[i] = data[static_cast<uint64_t>(point_id) * dim + i];
    }
    __syncthreads();
...
__global__ void
reorder_kernel(float *data, float *reorder_data, uint32_t *segment_indexes, uint32_t *map,
               uint32_t *segment_starts, uint32_t num, uint32_t dim) {
    uint32_t point_id = blockIdx.x;
    if (point_id >= num) {
        return;
    }
    ...
    for (uint32_t i = threadIdx.x; i < dim; i += blockDim.x) {
        reorder_data[reorder_id * dim + i] = data[point_id * dim + i];
    }
    if (threadIdx.x == 0) {
