...
namespace hashmap {
  ...
  const uint32_t stride = (key >> bitlen) * 2 + 1;
    ...
    __device__ inline void restore(uint32_t * hashmap, size_t hash_bit, uint32_t *beam_id_buffer,
                                   uint32_t clean_mask, uint32_t beam, uint32_t first_tid = 0){
        if (threadIdx.x < first_tid) return;
        for (unsigned i = threadIdx.x - first_tid; i < beam; i += blockDim.x - first_tid) {
            auto key = beam_id_buffer[i] & clean_mask;
