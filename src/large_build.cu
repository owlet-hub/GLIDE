...
int main(int argc, char **argv) {
    ...
    for(uint32_t i=0;i<number;i++){
        ...
        out.write(reinterpret_cast<const char *>(index.graph_view().data_handle() + start_pos), degree * sizeof(uint32_t));
    }
    out.close();
    ...
    start_point_out.write(reinterpret_cast<const char *>(index.start_point_view().data_handle()), start_point_size * sizeof(uint32_t));
}