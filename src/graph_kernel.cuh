...
__device__
void get_init(uint32_t *candidate_id_buffer, float *candidate_dist_buffer, float *query_buffer, uint32_t point_id,
              const float *data, const uint32_t *graph, uint32_t start_point, uint32_t dim, uint32_t knn_degree,
              uint32_t *visited_hashmap_ptr, uint32_t hash_bit, Metric metric) {
    for (uint32_t i = threadIdx.x; i < knn_degree; i += blockDim.x) {
        ...
        uint32_t flag = (index != get_max_value<uint32_t>()) ?
                        hashmap::insert(visited_hashmap_ptr, hash_bit, index) : 0;
...
__device__ void
compute_distance_for_build(uint32_t *candidate_id_buffer, float *candidate_dist_buffer, float *query_buffer,
...
__device__ void
get_neighbors(float *point_buffer, uint32_t point_id, const float *data, uint32_t dim, const uint32_t *knn_graph,
              uint32_t knn_degree, uint32_t beam, uint32_t *beam_id_buffer, float *beam_dist_buffer,
              ...
              uint32_t max_iterations, uint32_t min_iterations, uint32_t *select_buffer, Metric metric) {
    uint32_t iter = 0;
    ...
    while (true) {
        update_result_buff<MAX_BEAM, MAX_CANDIDATE>(beam_id_buffer, beam_dist_buffer, beam, candidate_id_buffer,
                                                    candidate_dist_buffer, knn_degree, workspace_buffer, (iter == 0),
        ...
        if ((iter + 1) % hash_reset_interval == 0) {
            ...
            hashmap::restore(visited_hashmap, hash_bit, beam_id_buffer, index_mask::clean, beam, first_tid);
        }
        __syncthreads();
...
__device__ void
sync_prune(const float *data, uint32_t segment_start, uint32_t dim, uint32_t *candidate_ids, float *candidate_dists,
    ...
    while (neighbor_size[0] < graph_degree && pool_size[0] > 0) {
        ...
        uint32_t id = candidate_ids[swizzling(0)];
        if (threadIdx.x == 0) {
        ...
        for (uint32_t i = warp_id + 1; i < pool_size[0]; i += warp_num) {
            uint32_t index = candidate_ids[swizzling(i)];
            float dist = compute_similarity_warp(data + (segment_start + id) * dim,
                                                 data + (segment_start + index) * dim, dim, metric);

            if (lane_id == 0) {
                uint32_t allow = (relaxant_factor * dist < candidate_dists[swizzling(i)]) ? 0 : 1;
        ...
        if (warp_id == 0) {
            ...
            for (uint32_t i = lane_id; i < bitmap_size * 32; i += warpSize) {
                ...
                uint32_t id_temp = (i < pool_size[0]) ? candidate_ids[swizzling(i)] : get_max_value<uint32_t>();
                float dist_temp = (i < pool_size[0]) ? candidate_dists[swizzling(i)] : get_max_value<float>();

                if (allow) {
                    const auto next = __popc(ballot_mask & ((1 << lane_id) - 1)) + allow_num;
                    candidate_ids[swizzling(next)] = id_temp;
                    candidate_dists[swizzling(next)] = dist_temp;
                }
...
//__device__ void
//sync_prune_boundary(const float *d_data, uint32_t segment_start, uint32_t dim, uint32_t *candidate_id,
//                    float *candidate_dist, uint32_t graph_degree, uint32_t max_graph_degree,
//                    uint32_t beam, uint32_t *neighbor_id,
//                    uint32_t *prune_args, uint32_t *bitmap, float relaxant_factor, Metric metric) {
//    uint32_t *neighbor_size = &prune_args[0];
//    uint32_t *pool_size = &prune_args[1];
//
//    uint32_t warp_id = threadIdx.x / warpSize;
//    uint32_t lane_id = threadIdx.x % warpSize;
//    uint32_t warp_num = blockDim.x / warpSize;
...
//    } else if (threadIdx.x >= 32){
//        for (unsigned i = threadIdx.x - 32; i < max_graph_degree; i += blockDim.x - 32) {
//            neighbor_id[i] = get_max_value<uint32_t>();
//        }
//    }
//    __syncthreads();
//
...
//        uint32_t bitmap_size = ceildiv<uint32_t>(pool_size[0], 32);
//
//        uint32_t id = candidate_id[swizzling(0)];
////        uint32_t id = candidate_id[0];
//        if (threadIdx.x == 0) {
//            neighbor_id[neighbor_size[0]] = id + segment_start;
//            neighbor_size[0]++;
//
//            for (uint32_t i = 0; i < bitmap_size; i++) {
//                bitmap[i] = 0;
...
//
//        for (uint32_t i = warp_id + 1; i < pool_size[0]; i += warp_num) {
//            uint32_t index = candidate_id[swizzling(i)];
////            uint32_t index = candidate_id[i];
//            float dist = compute_similarity_warp(d_data + (id + segment_start) * dim,
//                                                 d_data + (index + segment_start) * dim, dim, metric);
//
//            if (lane_id == 0) {
//                uint32_t allow = (relaxant_factor * dist < candidate_dist[swizzling(i)]) ? 0 : 1;
////                uint32_t allow = (relaxant_factor * dist < candidate_dist[i]) ? 0 : 1;
//                atomicOr(&bitmap[i / 32], (allow << (i % 32)));
//            }
//        }
//        __syncthreads();
...
//                const uint32_t ballot_mask = bitmap[bitmap_index];
//
//                uint32_t id_temp = (i < pool_size[0]) ? candidate_id[swizzling(i)] : get_max_value<uint32_t>();
//                float dist_temp = (i < pool_size[0]) ? candidate_dist[swizzling(i)] : get_max_value<float>();
////                uint32_t id_temp = (i < pool_size[0]) ? candidate_id[i] : get_max_value<uint32_t>();
////                float dist_temp = (i < pool_size[0]) ? candidate_dist[i] : get_max_value<float>();
//
//                if (allow) {
//                    const auto next = __popc(ballot_mask & ((1 << lane_id) - 1)) + num_allow;
//                    candidate_id[swizzling(next)] = id_temp;
//                    candidate_dist[swizzling(next)] = dist_temp;
////                    candidate_id[next] = id_temp;
////                    candidate_dist[next] = dist_temp;
//                }
//                num_allow += __popc(ballot_mask);
//            }
//            if (lane_id == 0) {
//                pool_size[0] = num_allow;
//            }
//        }
//        __syncthreads();
//    }
...
map_to_graph(uint32_t *d_graph, uint32_t *neighbors, uint32_t graph_degree) {
    for (uint32_t i = threadIdx.x; i < graph_degree; i += blockDim.x) {
        d_graph[i] = neighbors[i];
    }
}

__device__ inline void
map_to_graph_sub(uint32_t *d_graph, uint32_t *neighbors, const uint32_t *map, uint32_t graph_degree) {
    for (uint32_t i = threadIdx.x; i < graph_degree; i += blockDim.x) {
        d_graph[i] = (neighbors[i] == get_max_value<uint32_t>()) ? get_max_value<uint32_t>() : map[neighbors[i]];
    }
}

__device__ void
map_to_graph_boundary(uint32_t *d_graph, uint32_t *s_graph, uint32_t *neighbor_buffer, const uint32_t *map,
    ...
    for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
        s_graph[i] = (i < graph_degree) ? d_graph[i] : get_max_value<uint32_t>();
    }
    __syncthreads();
    ...
    if (repetitive_boundary_num >= graph_degree / 2) {
        ...
        for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
            ...
            if (valid_pos) {
                uint32_t position = __popc(ballot_mask & ((1 << threadIdx.x) - 1)) + num_valid_pos;
                if (position < valid_boundary_num) {
                    d_graph[i] = neighbor_buffer[position];
                }
            }
            num_valid_pos += __popc(ballot_mask);
            if (num_valid_pos >= valid_boundary_num) {
                break;
            }
        }
    } else {
        uint32_t max_need_size = graph_degree / 2 - repetitive_boundary_num;
        uint32_t need_size = (valid_boundary_num < max_need_size) ? valid_boundary_num : max_need_size;
        uint32_t num_valid_pos = 0;
        for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
            ...
            if (invalid || !repetitive) {
                uint32_t position = __popc(ballot_mask & ((1 << threadIdx.x) - 1)) + num_valid_pos;
                if (position < need_size) {
                    d_graph[graph_index] = neighbor_buffer[position];
                }
            }
            num_valid_pos += __popc(ballot_mask);
            if (num_valid_pos >= need_size) {
                break;
            }
        }
    }
...
template<uint32_t MAX_BEAM, uint32_t MAX_CANDIDATE>
__global__ void
build_sub_kernel(const float *data, uint32_t *d_graph, const uint32_t *start_points, const uint32_t *segment_start,
                 const uint32_t *segment_length, uint32_t knn_degree, uint32_t graph_degree, uint32_t max_graph_degree,
    ...
    for (segment_id = 0; segment_id < segment_num; segment_id++) {
        uint32_t segment_end = segment_start[segment_id] + segment_length[segment_id];
        if (point_id < segment_end) {
            break;
        }
    }
    ...
    get_init(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id,
             data + segment_start[segment_id] * dim, knn_graph + segment_start[segment_id] * knn_degree,
             start_points[segment_id], dim, knn_degree, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors<MAX_BEAM, MAX_CANDIDATE>(point_buffer, point_id - segment_start[segment_id],
                                           data + segment_start[segment_id] * dim, dim,
                                           knn_graph + segment_start[segment_id] * knn_degree, knn_degree, beam,
                                           result_id_buffer, result_dist_buffer, result_id_buffer + beam,
                                           result_dist_buffer + beam, terminate_flag, workspace_buffer,
                                           visited_hashmap, hash_bit, hash_reset_interval, max_iterations,
                                           min_iterations, select_buffer, metric);
    __syncthreads();

    sync_prune(data, segment_start[segment_id], dim, result_id_buffer, result_dist_buffer, graph_degree,
               max_graph_degree, beam, neighbor_buffer, prune_buffer, bitmap, relaxant_factor, metric);

    map_to_graph_sub(d_graph + map[point_id] * graph_degree, neighbor_buffer, map, graph_degree);
}

template<uint32_t MAX_BEAM, uint32_t MAX_CANDIDATE>
__global__ void
build_boundary_kernel(const float *data, uint32_t *d_graph, const uint32_t *start_points, uint32_t knn_degree,
    ...
    get_init(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id, data + segment_start * dim,
             knn_graph + segment_start * knn_degree, start_points[segment_id], dim, knn_degree, visited_hashmap,
             hash_bit, metric);
    __syncthreads();

    get_neighbors<MAX_BEAM, MAX_CANDIDATE>(point_buffer, point_id - segment_start, data + segment_start * dim, dim,
                                           knn_graph + segment_start * knn_degree, knn_degree, beam, result_id_buffer,
                                           result_dist_buffer, result_id_buffer + beam, result_dist_buffer + beam,
                                           terminate_flag, workspace_buffer, visited_hashmap, hash_bit,
                                           hash_reset_interval, max_iterations, min_iterations, select_buffer, metric);
    __syncthreads();

    sync_prune(data, segment_start, dim, result_id_buffer, result_dist_buffer, graph_degree,
               max_graph_degree, beam, neighbor_buffer, prune_buffer, bitmap, relaxant_factor, metric);

    map_to_graph_boundary(d_graph + map[point_id] * graph_degree, s_graph, neighbor_buffer, map,
                          graph_degree, max_graph_degree, bitmap, bitmap_size);
}

__global__ void
reverse_graph_kernel(uint32_t *const d_destination_nodes, uint32_t *const d_reverse_graph,
                     uint32_t *const d_reverse_graph_degree, uint32_t *d_graph,
                     uint32_t num, uint32_t graph_degree, uint32_t degree_id) {
    ...
    for (uint32_t i = thread_id; i < num; i += thread_num) {
        d_destination_nodes[i] = d_graph[i * graph_degree + degree_id];
    }

    for (uint32_t source_node = thread_id; source_node < num; source_node += thread_num) {
        uint32_t destination_node = d_destination_nodes[source_node];
        ...
        for (uint32_t i = 0; i < graph_degree; i++) {
            if (d_graph[destination_node * graph_degree + i] == source_node) {
                have = 1;
                break;
            }
        }
        ...
        uint32_t reverse_index = atomicAdd(d_reverse_graph_degree + destination_node, 1);
        if (reverse_index < graph_degree) {
            d_reverse_graph[destination_node * graph_degree + reverse_index] = source_node;
        }
    }
}

__global__ void
insert_reverse_kernel(uint32_t *d_graph, uint32_t *d_reverse_graph, uint32_t *d_reverse_graph_degree,
    ...
    for (uint32_t i = threadIdx.x; i < max_graph_degree; i += blockDim.x) {
        s_graph[i] = (i < graph_degree) ? d_graph[point_id * graph_degree + i] : get_max_value<uint32_t>();
    }

    uint32_t num_reverse = (d_reverse_graph_degree[point_id] > graph_degree) ?
                           graph_degree : d_reverse_graph_degree[point_id];
    ...
    for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
        ...
        if (have_position) {
            ...
            if (position < num_reverse) {
                d_graph[point_id * graph_degree + i] = d_reverse_graph[point_id * graph_degree + position];
            }
        }
...
__device__ void
select_point(uint32_t *candidate_ids, uint32_t pool_size, uint32_t point_id,
    ...
    for (uint32_t i = threadIdx.x; i < roundUp32(pool_size); i += warpSize) {
        uint32_t id = (i < pool_size) ? candidate_ids[swizzling(i)] : get_max_value<uint32_t>();
...
__device__ void
sync_prune_refine(const float *data, uint32_t point_id, uint32_t dim, uint32_t *candidate_ids, float *candidate_dists,
                  uint32_t graph_degree, uint32_t beam, uint32_t *neighbor_buffer, uint32_t *s_graph,
                  uint32_t *prune_buffer, uint32_t *bitmap, float relaxant_factor, Metric metric) {
    uint32_t *neighbor_size = &prune_buffer[0];
    uint32_t *pool_size = &prune_buffer[1];
    uint32_t *need_size = &prune_buffer[2];
    ...
    while (neighbor_size[0] < need_size[0] && pool_size[0] > 0) {
        uint32_t bitmap_size = ceildiv<uint32_t>(pool_size[0], 32);;

        if (warp_id == 0) {
            select_point(candidate_ids, pool_size[0], point_id, position, s_graph, graph_degree);
        }
        __syncthreads();
        ...
        uint32_t id = candidate_ids[swizzling(position[0])];
        if (threadIdx.x == 0) {
...
__global__ void
refine_kernel(const float *data, uint32_t *d_graph, const uint32_t *start_points, const float *centroids,
              ...
              uint32_t centroid_num, float relaxant_factor, uint32_t offset, Metric metric) {
    uint32_t point_id = blockIdx.x + offset;
    if (point_id >= num) {
        return;
    }
    ...
    if (threadIdx.x < 32) {
        ...
        for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
            ...
            need_num += __popc(ballot_mask);
        }
        if (threadIdx.x == 0) {
            need_size[0] = need_num;
        }
    }
    __syncthreads();
    ...
    result_init<MAX_CENTROID>(result_id_buffer, result_dist_buffer, point_buffer, result_id_buffer + beam,
                              result_dist_buffer + beam, data, d_graph, start_points, centroids, dim, graph_degree,
                              centroid_num, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors<MAX_BEAM, MAX_CANDIDATE>(point_buffer, point_id, data, dim, d_graph, graph_degree, beam,
                                           result_id_buffer, result_dist_buffer, result_id_buffer + beam,
                                           result_dist_buffer + beam, terminate_flag, workspace_buffer, visited_hashmap,
                                           hash_bit, hash_reset_interval, max_iterations, min_iterations,
    ...
    if (threadIdx.x < 32) {
        ...
        for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
            uint32_t valid = (i < graph_degree && s_graph[i] == get_max_value<uint32_t>()) ? 1 : 0;
            uint32_t ballot_mask = __ballot_sync(warp_full_mask(), valid);
            if (valid) {
                uint32_t can_position = __popc(ballot_mask & ((1 << threadIdx.x) - 1)) + valid_num;
                d_graph[point_id * graph_degree + i] = neighbor_buffer[can_position];
            }
...
__global__
void reverse_start_id_kernel(uint32_t *graph, uint32_t segment_num, uint32_t *segment_start, uint32_t *segment_length,
                             uint32_t num, uint32_t graph_degree) {
    uint32_t point_id = blockIdx.x;
    if (point_id >= num) {
        return;
    }
    ...
    for (uint32_t i = threadIdx.x; i < graph_degree; i += blockDim.x) {
        graph[point_id * graph_degree + i] -= segment_start[segment_id];
    }
}

__device__
void get_init_for_large(uint32_t *candidate_id_buffer, float *candidate_dist_buffer, uint8_t *point_buffer,
                        uint32_t point_id, const uint8_t *data, const uint32_t *graph, uint32_t start_point,
                        uint32_t dim, uint32_t knn_degree, uint32_t *visited_hashmap_ptr, uint32_t hash_bit,
                        Metric metric) {
    for (uint32_t i = threadIdx.x; i < knn_degree; i += blockDim.x) {
        uint32_t index = graph[static_cast<uint64_t>(start_point) * knn_degree + i];
        uint32_t flag = (index != get_max_value<uint32_t>()) ? hashmap::insert(visited_hashmap_ptr, hash_bit, index)
                                                             : 0;
        candidate_id_buffer[i] = (flag == 1) ? index : get_max_value<uint32_t>();
    }
    __syncthreads();
...
compute_distance_for_large(uint32_t *candidate_id_buffer, float *candidate_dist_buffer, uint8_t *query_buffer,
    ...
    for (uint32_t i = warp_id; i < knn_degree; i += warp_num) {
        uint32_t index = candidate_id_buffer[i];
        float dist = (index != get_max_value<uint32_t>()) ?
                     compute_similarity_warp(query_buffer, data + static_cast<uint64_t>(index) * dim, dim, metric) :
                     get_max_value<float>();
        if (lane_id == 0) {
            candidate_dist_buffer[i] = dist;
        }
...
template<uint32_t MAX_BEAM, uint32_t MAX_CANDIDATE>
__device__ void
get_neighbors_for_large(uint8_t *point, uint32_t point_id, const uint8_t *data, uint32_t dim, const uint32_t *knn_graph,
                        uint32_t knn_degree, uint32_t beam, uint32_t *beam_id_buffer, float *beam_dist_buffer,
                        uint32_t *candidate_id_buffer, float *candidate_dist_buffer, uint32_t *terminate_flag,
                        ...
                        uint32_t *select_buffer, Metric metric) {
    uint32_t iter = 0;
    const uint32_t multi_warps_1 = ((blockDim.x >= 64) && (MAX_CANDIDATE > 128)) ? 1 : 0;
    const uint32_t multi_warps_2 = ((blockDim.x >= 64) && (MAX_BEAM > 256)) ? 1 : 0;
    while (true) {
        ...
        if ((iter + 1) % hash_reset_interval == 0) {
            uint32_t first_tid = ((blockDim.x <= 32) ? 0 : 32);
            hashmap::restore(visited_hashmap, hash_bit, beam_id_buffer, beam, first_tid);
        }
        __syncthreads();
        ...
        compute_distance_for_large(candidate_id_buffer, candidate_dist_buffer, point, point_id, knn_graph, knn_degree,
                                   visited_hashmap, hash_bit, select_buffer, dim, data, metric);
        __syncthreads();
...
__device__ void
sync_prune_for_large(const uint8_t *data, uint32_t dim, uint32_t *candidate_ids, float *candidate_dists,
                     uint32_t graph_degree, uint32_t max_graph_degree, uint32_t beam, uint32_t *neighbor_buffer,
                     uint32_t *prune_buffer, uint32_t *bitmap, float relaxant_factor,
...
template<uint32_t MAX_BEAM, uint32_t MAX_CANDIDATE>
__global__ void
build_for_large_kernel(const uint8_t *data, uint32_t *d_graph, uint32_t start_point, uint32_t knn_degree,
    ...
    get_init_for_large(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id,
                       data, knn_graph, start_point, dim, knn_degree, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors_for_large<MAX_BEAM, MAX_CANDIDATE>(point_buffer, point_id, data, dim, knn_graph, knn_degree, beam,
                                                     result_id_buffer, result_dist_buffer, result_id_buffer + beam,
                                                     result_dist_buffer + beam, terminate_flag, workspace_buffer,
                                                     visited_hashmap, hash_bit, hash_reset_interval,
    ...
    sync_prune_for_large(data, dim, result_id_buffer, result_dist_buffer, graph_degree, max_graph_degree, beam,
                         neighbor_buffer, prune_buffer, bitmap, relaxant_factor, metric, point_id);

    map_to_graph(d_graph + static_cast<uint64_t>(point_id) * graph_degree, neighbor_buffer, graph_degree);
}

__device__ void
sync_prune_refine_for_large(const uint8_t *data, uint32_t point_id, uint32_t dim, uint32_t *candidate_ids,
                            float *candidate_dists, uint32_t graph_degree, uint32_t beam, uint32_t *neighbor_buffer,
...
template<uint32_t MAX_BEAM, uint32_t MAX_CANDIDATE>
__global__ void
refine_for_large_kernel(const uint8_t *data, uint32_t *d_graph, uint32_t start_point, uint32_t graph_degree,
    ...
    get_init_for_large(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id,
                       data, d_graph, start_point, dim, graph_degree, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors_for_large<MAX_BEAM, MAX_CANDIDATE>(point_buffer, point_id, data, dim, d_graph, graph_degree, beam,
                                                     result_id_buffer, result_dist_buffer, result_id_buffer + beam,
                                                     result_dist_buffer + beam, terminate_flag, workspace_buffer,
                                                     visited_hashmap, hash_bit, hash_reset_interval,
                                                     max_iterations, min_iterations, select_buffer, metric);
