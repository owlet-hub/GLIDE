...
template<uint32_t MAX_CANDIDATE, uint32_t MAX_BEAM, typename Data_t, typename Index_t>
__device__ void
get_neighbors(Data_t *point_buffer, uint32_t point_id, const Data_t *data, uint32_t dim, uint32_t beam,
    ...
    while (true) {
        ...
        compute_distance<Data_t, Index_t>(candidate_id_buffer, candidate_dist_buffer, point_buffer, point_id, knn_graph,
...
__device__ void
sync_prune(const Data_t *data, uint32_t segment_start, uint32_t dim, uint32_t *beam_id_buffer, float *beam_dist_buffer,
    ...
    while (neighbor_size[0] < graph_degree && pool_size[0] > 0) {
        ...
        uint32_t id = beam_id_buffer[swizzling(0)] + segment_start;
        if (threadIdx.x == 0) {
        ...
        for (uint32_t i = warp_id + 1; i < pool_size[0]; i += warp_num) {
            uint32_t index = beam_id_buffer[swizzling(i)] + segment_start;
            float dist = compute_similarity_warp(data + static_cast<Index_t>(id) * dim,
                                                 data + static_cast<Index_t>(index) * dim, dim, metric);
...
map_to_graph(uint32_t *graph, const uint32_t *neighbor_buffer, uint32_t graph_degree) {
...
__device__ inline void
map_to_graph_sub(uint32_t *graph, const uint32_t *neighbor_buffer, const uint32_t *map, uint32_t graph_degree) {
    for (uint32_t i = threadIdx.x; i < graph_degree; i += blockDim.x) {
...
template<uint32_t MAX_CANDIDATE, uint32_t MAX_BEAM, typename Data_t, typename Index_t>
__global__ void
build_kernel(const Data_t *data, uint32_t *graph, const uint32_t *start_points, const uint32_t *segment_starts,
    ...
    get_init<Data_t, Index_t>(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id,
                              data + segment_starts[segment_id] * dim,
                              knn_graph + segment_starts[segment_id] * knn_degree,
                              start_points[segment_id], dim, knn_degree, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors<MAX_CANDIDATE, MAX_BEAM, Data_t, Index_t>(point_buffer, point_id - segment_starts[segment_id],
                                                            data + segment_starts[segment_id] * dim, dim, beam,
                                                            knn_graph + segment_starts[segment_id] * knn_degree,
                                                            result_id_buffer, result_dist_buffer,
                                                            result_id_buffer + beam, result_dist_buffer + beam,
                                                            terminate_flag, workspace_buffer, visited_hashmap, hash_bit,
                                                            hash_reset_interval, knn_degree, max_iterations,
                                                            min_iterations, select_buffer, metric);
    __syncthreads();

    sync_prune<Data_t, Index_t>(data, segment_starts[segment_id], dim, result_id_buffer, result_dist_buffer,
                                graph_degree, max_graph_degree, beam, neighbor_buffer, prune_buffer, bitmap,
                                relaxant_factor, metric);
...
template<uint32_t MAX_CANDIDATE, uint32_t MAX_BEAM, typename Data_t, typename Index_t>
__global__ void
build_boundary_kernel(const Data_t *data, uint32_t *graph, const uint32_t *start_points, uint32_t knn_degree,
    ...
    get_init<Data_t, Index_t>(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id,
                              data + segment_start * dim, knn_graph + segment_start * knn_degree,
                              start_points[segment_id], dim, knn_degree, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors<MAX_CANDIDATE, MAX_BEAM, Data_t, Index_t>(point_buffer, point_id - segment_start,
                                                            data + segment_start * dim, dim, beam,
                                                            knn_graph + segment_start * knn_degree, result_id_buffer,
    ...
    sync_prune<Data_t, Index_t>(data, segment_start, dim, result_id_buffer, result_dist_buffer, graph_degree,
...
reverse_graph_kernel(uint32_t *const destination_nodes, uint32_t *const reverse_graph,
                     uint32_t *const reverse_graph_degrees, const uint32_t *graph, uint32_t num,
    ...
    for (uint32_t i = thread_id; i < num; i += thread_num) {
        destination_nodes[i] = graph[static_cast<Index_t>(i) * graph_degree + degree_id];
    }
    ...
    for (uint32_t source_node = thread_id; source_node < num; source_node += thread_num) {
        ...
        for (uint32_t i = 0; i < graph_degree; i++) {
            if (graph[static_cast<Index_t>(destination_node) * graph_degree + i] == source_node) {
                have = 1;
                break;
            }
        }
...
template<typename Index_t>
__global__ void
insert_reverse_kernel(uint32_t *graph, const uint32_t *reverse_graph, const uint32_t *reverse_graph_degrees,
    ...
    for (uint32_t i = threadIdx.x; i < max_graph_degree; i += blockDim.x) {
        s_graph[i] = (i < graph_degree) ? graph[static_cast<Index_t>(point_id) * graph_degree + i] :
    ...
    for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
        ...
        if (have_position) {
            ...
            if (position < num_reverse) {
                graph[static_cast<Index_t>(point_id) * graph_degree + i] =
                        reverse_graph[static_cast<Index_t>(point_id) * graph_degree + position];
            }
        }
...
__device__ void
select_point(const uint32_t *beam_id_buffer, uint32_t pool_size, uint32_t point_id,
             uint32_t *position, const uint32_t *s_graph, uint32_t graph_degree) {
    if (threadIdx.x == 0) {
...
template<typename Data_t, typename Index_t>
__device__ void
sync_prune_refine(const Data_t *data, uint32_t point_id, uint32_t dim, uint32_t *beam_id_buffer,
                  float *beam_dist_buffer, uint32_t graph_degree, uint32_t beam, uint32_t *neighbor_buffer,
                  uint32_t *s_graph, uint32_t *prune_buffer, uint32_t *bitmap, float relaxant_factor, Metric metric) {
    uint32_t *neighbor_size = &prune_buffer[0];
    uint32_t *pool_size = &prune_buffer[1];
    ...
    while (neighbor_size[0] < need_num[0] && pool_size[0] > 0) {
        ...
        if (warp_id == 0) {
            select_point(beam_id_buffer, pool_size[0], point_id, position, s_graph, graph_degree);
        }
        __syncthreads();
...
template<uint32_t MAX_CANDIDATE, uint32_t MAX_BEAM, uint32_t MAX_CENTROID, typename Data_t, typename Index_t>
__global__ void
refine_kernel(const Data_t *data, uint32_t *graph, const uint32_t *start_points, const float *centroids,
    ...
    result_init<MAX_CENTROID, Data_t, Index_t>(result_id_buffer, result_dist_buffer, result_id_buffer + beam,
                                               result_dist_buffer + beam, point_buffer, data, graph, start_points,
                                               centroids, dim, graph_degree, segment_num, visited_hashmap, hash_bit,
                                               metric);
    __syncthreads();

    get_neighbors<MAX_CANDIDATE, MAX_BEAM, Data_t, Index_t>(point_buffer, point_id, data, dim, beam, graph,
                                                            result_id_buffer, result_dist_buffer,
                                                            result_id_buffer + beam, result_dist_buffer + beam,
                                                            terminate_flag, workspace_buffer, visited_hashmap, hash_bit,
                                                            hash_reset_interval, graph_degree, max_iterations,
                                                            min_iterations, select_buffer, metric);
    __syncthreads();

    sync_prune_refine<Data_t, Index_t>(data, point_id, dim, result_id_buffer, result_dist_buffer, graph_degree, beam,
                                       neighbor_buffer, s_graph, prune_buffer, bitmap, relaxant_factor, metric);
    __syncthreads();
    ...
    if (threadIdx.x < 32) {
        ...
        for (uint32_t i = threadIdx.x; i < max_graph_degree; i += warpSize) {
            ...
            if (valid_pos) {
                ...
                graph[static_cast<Index_t>(point_id) * graph_degree + i] = neighbor_buffer[can_position];
            }
            valid_pos_num += __popc(ballot_mask);
        }
    }
...
template<uint32_t MAX_CANDIDATE, uint32_t MAX_BEAM, typename Data_t, typename Index_t>
__global__ void
build_for_large_kernel(const Data_t *data, uint32_t *graph, uint32_t start_point, uint32_t knn_degree,
    ...
    get_init<Data_t, Index_t>(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id, data,
                              knn_graph, start_point, dim, knn_degree, visited_hashmap, hash_bit, metric);
    __syncthreads();

    get_neighbors<MAX_CANDIDATE, MAX_BEAM, Data_t, Index_t>(point_buffer, point_id, data, dim, beam, knn_graph,
                                                            result_id_buffer, result_dist_buffer,
                                                            result_id_buffer + beam, result_dist_buffer + beam,
    ...
    sync_prune<Data_t, Index_t>(data, 0, dim, result_id_buffer, result_dist_buffer, graph_degree, max_graph_degree,
                                beam, neighbor_buffer, prune_buffer, bitmap, relaxant_factor, metric);
...
template<uint32_t MAX_CANDIDATE, uint32_t MAX_BEAM, typename Data_t, typename Index_t>
__global__ void
refine_for_large_kernel(const Data_t *data, uint32_t *graph, uint32_t start_point, uint32_t graph_degree,
    ...
    get_init<Data_t, Index_t>(result_id_buffer + beam, result_dist_buffer + beam, point_buffer, point_id, data, graph,
