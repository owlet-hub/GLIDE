...
get_centroids(raft::device_resources &handle,
              ...
              raft::device_matrix_view<float, int> d_centroids_view,
    ...
    kmeans::fit(handle, params, d_sample_data.view(), std::nullopt, d_centroids_view,
...
void get_centroids_for_large(raft::device_resources &handle,
    ...
    for (uint32_t i = 0; i < batch_num; i++) {
        ...
        raft::copy(d_data_view.data_handle(), h_data_view.data_handle() + static_cast<uint64_t>(start_id) * dim,
                   static_cast<uint64_t>(batch_size) * dim, stream);

        cudaEventRecord(start_time);
        sample_for_large_kernel<<<blocks_per_grid, threads_per_block, 0, stream>>>(d_data_view.data_handle(),
                                                                                   d_sample_data.data_handle(),
                                                                                   d_indices.data_handle(),
                                                                                   dim,
                                                                                   sample_num,
...
void define_partition(raft::device_resources &handle,
                      raft::device_matrix_view<float> d_data_view,
                      raft::device_matrix_view<float> d_centroids_view,
                      ...
                      float boundary_factor, Metric metric, float &build_time) {
    uint32_t centroid_num = d_centroids_view.extent(0);
    uint32_t num = d_data_view.extent(0);
    uint32_t dim = d_data_view.extent(1);
    ...
    cudaEventRecord(start_time);
    kernel<<<blocks_per_grid, threads_per_block, shared_mem_size, stream>>>(d_data_view.data_handle(),
                                                                            d_centroids_view.data_handle(),
                                                                            num,
                                                                            dim,
                                                                            centroid_num,
...
void reorder(raft::device_resources &handle,
             raft::device_matrix_view<float> d_data_view,
...
void reorder_for_large(raft::device_resources &handle,
                       raft::host_matrix_view<uint8_t, uint64_t> h_data_view,
                       raft::device_matrix_view<uint8_t, uint64_t> d_data_view,
    ...
    for (uint32_t i = 0; i < batch_num; i++) {
        ...
        for (uint32_t segment_id = 0; segment_id < segment_num; segment_id++) {
            ...
            cudaEventRecord(start_time);
            reorder_for_large_kernel<<<blocks_per_grid, threads_per_block, 0, stream>>>(d_data_view.data_handle(),
            ...
            uint32_t start_index = segment_start + current_counts[segment_id];
            uint32_t count = h_counts(segment_id);
            raft::copy(h_map_view.data_handle() + start_index, d_batch_map.data_handle(),
                       count, raft::resource::get_cuda_stream(handle));
            raft::copy(h_reorder_data_view.data_handle() + static_cast<uint64_t>(start_index) * dim,
                       d_batch_data.data_handle(),
                       static_cast<uint64_t>(count) * dim, raft::resource::get_cuda_stream(handle));

            current_counts[segment_id] += count;
        }
    }
}

void preprocess(raft::device_resources &handle,
                PartitionParameter param,
                raft::device_matrix_view<float> d_data_view,
    ...
    get_centroids(handle, d_data_view, d_centroids_view, param.centroid_num, param.sample_factor, build_time);

    define_partition(handle, d_data_view, d_centroids_view, h_segment_start_view, h_segment_length_view,
...
void preprocess_for_large(raft::device_resources &handle,
                          PartitionParameter param,
                          raft::host_matrix_view<uint8_t, uint64_t> h_data_view,
                          raft::device_matrix_view<uint8_t, uint64_t> d_data_view,
                          raft::host_matrix_view<uint8_t, uint64_t> h_reorder_data_view,
    ...
    get_centroids_for_large(handle, h_data_view, d_data_view, d_centroids_view,
                            param.centroid_num, param.sample_factor, batch_num, build_time);

    define_partition_for_large(handle, h_data_view, d_data_view, d_centroids_view,
                               h_segment_start_view, h_segment_length_view, d_segment_index.view(),
                               batch_num, param.metric, build_time);

    reorder_for_large(handle, h_data_view, d_data_view, h_map_view, h_reorder_data_view,
                      h_segment_start_view, d_segment_index.view(), batch_num, build_time);
